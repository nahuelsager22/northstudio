...
**Metadata.** Toda página pasa por un solo constructor, `metadataDe` (`src/lib/metadata.ts`): declara lo que es suyo —título, descripción y su dirección en cada idioma— y recibe igual para todas la base absoluta, la tarjeta de previsualización, el locale de Open Graph, `canonical` y `hreflang`. Existe por cómo fusiona Next: `openGraph` y `alternates` se reemplazan, no se combinan, así que una página que definía uno perdía el otro y una que no definía ninguno heredaba el del layout entero (`canonical` a la portada incluido). `metadataBase` sale de `src/lib/sitio.ts`, que deriva la URL del entorno: variable explícita → Netlify (`URL` en producción, `DEPLOY_PRIME_URL` en previews) → Vercel → localhost. **El sitio vive en Netlify.** El sitemap sale del mismo loader que la interfaz, así que un borrador no aparece.
