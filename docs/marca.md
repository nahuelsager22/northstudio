...
Lo que aparece al compartir un enlace del sitio en WhatsApp, Instagram, iMessage, Slack o cualquier lector de Open Graph. Se genera con `node scripts/og.mjs` y sale a `public/og.png` (1200×630); la declara `metadataDe` (`src/lib/metadata.ts`) para todas las páginas, con ancho, alto y texto alternativo traducido.
...
*Lo que la rompió el primer día, para que no vuelva a pasar:* la imagen se declaraba bien pero la URL absoluta salía como `http://localhost:3000/og.png` en producción, porque `sitio.ts` sólo conocía las variables de Vercel y el sitio está en Netlify. WhatsApp no mostraba nada e Instagram seguía con su vista anterior. Si una tarjeta no aparece, lo primero es leer el `og:image` del HTML publicado con `curl`, antes que cualquier caché.
