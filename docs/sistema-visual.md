...
- **El recorrido es una atmósfera, no un fondo.** El sitio no tiene un color de fondo ni tres puestos uno debajo del otro: tiene **un descenso de color y una vuelta**, y ese descenso es el ritmo.
  ...
  ```
  cielo  →  onda  →  bosque  →  onda  →  liquen  →  onda  →  tierra  →  onda  →  cielo
  ```
...
- **La única transición es la onda.** No hay degradados entre planos: color plano, onda, color plano. Un fundido hacía dos trabajos —presentar dos colores que no se conocían y disolver el borde—; la onda ya disuelve el borde, y presentar los colores dejó de hacer falta cuando los colores pasaron a ser de la misma familia. Lo que quedaba era un fundido entre el cielo y el suelo, que es la única cosa que un lugar no tiene.
- **Las cuatro superficies.** `cielo` es el fondo del tema (con estrellas si es Noche) y sostiene el umbral **y el pie**; `bosque` —`oklch(0.347 0.049 140)`, #2C4027— sostiene el trabajo y la apertura de un caso; `liquen` —`oklch(0.775 0.032 134)`, #AEBBA6— sostiene el estudio; `tierra` —`oklch(0.325 0.033 66)`, #403122— sostiene la conversación. **Las tres de color son iguales en los dos temas:** el tema gobierna sólo el cielo.
- **La paleta se sostiene sola: nada está saturado, los matices hacen un solo arco y la luz sube y baja simétrica.** Los cuatro cromas caben entre 0.018 y 0.049 — un color sofisticado es casi siempre un color que se contuvo. Los matices 265 → 140 → 134 → 66 bajan siempre por el mismo lado de la rueda (azul, verde, pardo), así que ningún par de vecinos cruza el punto muerto del medio. Y las luminancias 0.115 → 0.347 → 0.775 → 0.325 → 0.115 hacen un día: **bosque y tierra están a la misma altura, un verde y un pardo** — la misma profundidad vista desde dos temperaturas. El bosque y el liquen son además el mismo verde a dos alturas, y cada uno es la voz baja del otro (#B2BFAC sobre bosque es el liquen; #2C4027 sobre liquen es el bosque).
- **Una superficie no es un color de fondo: es una paleta chica.** Cada bloque `[data-superficie]` redeclara los seis tokens (y su espejo `--color-*`, porque `@theme inline` resuelve `--color-ink: var(--ink)` en `:root` y lo que se hereda es el valor ya resuelto). Por eso un plano de liquen vive dentro de un sitio de noche sin que ninguna clase de texto sepa dónde está.
- **Una transición es una pieza, no un borde.** `components/transicion.tsx`: un bloque propio en el flujo, del color de origen, con la curva del color de destino apoyada abajo. Un horizonte no es horizontal, y una recta a todo el ancho de la ventana es lo que hace que un cambio de fondo se lea como el final de un bloque. Las curvas se alternan: la misma onda tres veces sería un separador. **Dos modos, según de qué lado esté el cielo:** `sube` (el plano que llega asciende dentro del anterior) y `revela` (el que llega es el cielo, así que se pinta lo contrario — la masa del origen cortada por la curva— y debajo no queda nada).
