...
- **Y las dos líneas de créditos volvieron de mono a serif** — el colofón del estudio y la ficha de un caso. Una lista de servicios y una lista de roles no son notación: son los créditos de un impreso, y un impreso compone sus créditos en la tipografía del texto, en chico. En monoespaciada eran el único elemento de esas páginas que decía interfaz, y en el caso —cuatro roles seguidos— leían directamente como la salida de una terminal. La notación sigue siendo mono donde de verdad lo es: los rótulos del formulario, el aviso de borrador, el año de la lista, los epígrafes y las atribuciones.
...
- **El recorrido es una atmósfera, no un fondo.** El sitio no tiene un color de fondo ni tres puestos uno debajo del otro: tiene **un descenso de color y una vuelta**, y ese descenso es el ritmo.
  ...
  ```
  cielo  ↘  bosque  ↙  piedra  ↘  tierra  ↙  cielo
  ```
...
- **La única transición es un corte diagonal.** No hay degradados ni curvas entre planos: color plano, corte, color plano. Un fundido hacía dos trabajos —presentar dos colores que no se conocían y disolver el borde— y sobraba en los dos: los colores ya son de la misma familia, y disolver un borde entre el cielo y el suelo es la única cosa que un lugar no tiene.

  ~~Hasta la novena iteración el corte era una onda orgánica.~~ Funcionaba —no se cambió por un defecto— pero era también la firma visual del primer sitio de cliente publicado, y con sólo dos sitios en la calle una firma compartida se lee al revés de como es. **Una onda es un motivo —dibuja agua, colinas, un horizonte— y un motivo se hereda; una diagonal no dibuja nada: es una decisión de compaginación.** El estudio se queda con la gramática y cada cliente se queda con sus imágenes. Se evaluaron también el difuminado atmosférico —descartado porque es el lenguaje de otro proyecto de cliente, así que mueve el problema en vez de resolverlo, y porque reabre el degradado que la iteración anterior había sacado— y el sistema mixto recto+diagonal, descartado porque un corte perfectamente recto a todo el ancho es justamente lo que hace que un cambio de fondo se lea como el final de un bloque.
- **Las cuatro superficies.** `cielo` es el fondo del tema (con estrellas si es Noche) y sostiene el umbral **y el pie**; `bosque` —`oklch(0.274 0.017 163)`, #202A25— sostiene el trabajo y la apertura de un caso; `piedra` —`oklch(0.700 0.025 91)`, #A49E8D— sostiene el estudio; `tierra` —`oklch(0.303 0.016 43)`, #362C28— sostiene la conversación. **Las tres de color son iguales en los dos temas:** el tema gobierna sólo el cielo.

- **Un plano de tono medio comprime sus dos voces, y hay que compensarlo a mano.** Relación entre el contraste de la tinta y el de la voz baja, plano por plano: cielo 2.17, bosque 2.35, tierra 2.27 — la piedra llegó a estar en **1.30**, y esa sección se leía apagada no por el negro sino porque su texto secundario sonaba casi tan fuerte como el primero. La causa es el fondo: sobre L 0.700 la tinta no pasa de ~7.2:1, así que para que la voz baja siga siendo legible hay que subirla hasta casi tocarla. Se compensa de dos maneras y las dos hacen falta: **separando por temperatura** (la voz baja de la piedra es un pardo cálido, no un verde neutro que sobre caqui lee gris) y **abriendo la distancia hasta el mínimo AA** (4.57:1, que lleva la relación a 1.57). Lo que falta se resuelve en la tipografía, nunca bajando de AA.
- **La paleta se sostiene sola: nada está saturado, los matices hacen un solo arco y la luz sube y baja simétrica.** Los cuatro cromas caben entre 0.016 y 0.025 — un color sofisticado es casi siempre un color que se contuvo, y en campos grandes y planos un croma de 0.02 se ve perfectamente: la superficie hace el trabajo que en una muestra chica hay que hacer con saturación. Los matices 265 → 163 → 91 → 43 bajan siempre por el mismo lado de la rueda (azul, verde, caqui, pardo) y ninguno repite al anterior. Y las luminancias 0.115 → 0.274 → 0.700 → 0.303 → 0.115 hacen un día: **bosque y tierra están casi a la misma altura, un verde y un pardo** — la misma profundidad vista desde dos temperaturas, con el momento de luz entre medio.

  **Cada plano habla con el color de otro:** la voz baja del bosque es la piedra y la de la piedra es el bosque; la de la tierra vuelve a ser la piedra. El plano que viene ya se está hablando en el que se está.

  Los dos costos, dichos: la piedra a L 0.700 es un tono medio, así que su tinta no pasa de ~7.2:1 por más negra que se la ponga (AA en todo, AAA en títulos, pero es el techo más bajo de la página — el precio de que el momento de luz sea un material y no un papel). Y en Noche el escalón del cielo al bosque queda en 1.38:1, el más chico del recorrido: **lo carga la textura, no el color** — arriba del corte hay estrellas y abajo no. Es el único paso donde el cielo está arriba, así que es el único que puede permitírselo; en Papel el mismo escalón es enorme.
- **Una superficie no es un color de fondo: es una paleta chica.** Cada bloque `[data-superficie]` redeclara los seis tokens (y su espejo `--color-*`, porque `@theme inline` resuelve `--color-ink: var(--ink)` en `:root` y lo que se hereda es el valor ya resuelto). Por eso un plano de piedra vive dentro de un sitio de noche sin que ninguna clase de texto sepa dónde está.
- **Una transición es una pieza, no un borde.** `components/transicion.tsx`: un bloque propio en el flujo, del color de origen, con una cuña del color de destino apoyada abajo. Un horizonte no es horizontal, y una recta a todo el ancho de la ventana es lo que hace que un cambio de fondo se lea como el final de un bloque. **Dos rellenos, según de qué lado esté el cielo:** `destino` (el plano que llega entra desde abajo) y `origen` (el que llega es el cielo, así que se pinta lo contrario — la masa del origen cortada en diagonal— y debajo no queda nada).

- **El sesgo alterna, y se declara en la página y no en la transición.** Derecha, izquierda, derecha, izquierda: la dirección de un corte no es una propiedad del corte, es su lugar en la serie. Lo que se ve no son las diagonales sino los planos que quedan entre ellas — cada uno se inclina hacia el lado contrario que el anterior. **Cuatro paralelas serían un patrón, y un patrón se lee como plantilla;** alternadas, el descenso tiene ritmo en vez de deriva. Todas suben lo mismo (`--corte-alto`, de 3rem a 6rem): la alternancia ya es la variación, y agregar una segunda no se lee como composición sino como descuido.

- **El ángulo lo decide el formato, la subida es lo que se mantiene.** El SVG se estira sin conservar proporción, así que `--corte-alto` *es* la subida en píxeles: 3.8° en un escritorio de 1440, 7.3° en un teléfono de 375. Un mismo ángulo daría un sesgo invisible en el teléfono o un tajo en el escritorio; lo constante es cuánto se corre el borde, que es lo que se percibe.
...
- **Un plano tiene que poder leerse como plano.** `.plano`: una pantalla más la altura de una transición. Un color que nunca llena el cuadro —siempre con un corte arriba o abajo— no es un plano, es una franja entre dos bordes. El aire de sobra va abajo y no repartido: un ancla tiene que aterrizar en el contenido, no en el vacío de arriba.
- **Un plano y una escena aterrizan a ras.** El retiro por defecto de un ancla (siete rem) haría aterrizar dentro de la transición anterior, y lo que se vería arriba es el plano de antes. El aire que hace falta para no quedar bajo la nav vive adentro, en el `padding-top` del plano.
- **Un trabajo se presenta como una lámina, no como una captura en una caja.** Sin paspartú: sobre un plano de color, una imagen con aire alrededor ya es una lámina apoyada. Queda un keyline —una línea sin relleno ni aire— que garantiza el borde pase lo que pase adentro. La imagen y el nombre comparten eje, y la imagen no llega al margen derecho: lo que dice que algo está puesto a propósito es el espacio que se le dejó al lado, no el marco. Los bordes rectos de la lámina contra las curvas del plano son el contraste que la sostiene — y la otra razón para no enmarcarla.
...
- **La nav calcula el color que tiene detrás, cuadro a cuadro — y sobre el corte no pinta nada.** Cada tramo declara sus cuatro paradas (`--z0..--z3`) y el encabezado evalúa en qué punto del tramo cae su altura; la mezcla la hace `color-mix` en CSS. Sobre un plano liso las cuatro paradas son iguales, así que el velo y el plano son el mismo color y el velo no se ve. Sobre una transición el velo **no se pinta**: ahí no hay un color que heredar —la diagonal deja un color distinto en cada punto del ancho— y pintar el promedio devuelve la banda que el velo existe para no ser. La tinta es lo único que no se mezcla: salta una vez, donde el fondo ya cambió de mayoría — el fondo puede ser continuo, el contraste no puede.

- **El menú abierto también pertenece al plano.** El panel móvil declara la superficie que la nav está midiendo, así que su fondo y sus tintas salen de los tokens de ese plano y no de la raíz. Sobre el cielo eso no alcanza —**el cielo es color y estrellas**, y pintar sólo el color da un panel del color del cielo—: mientras dura ese estado el campo de estrellas, que está fijo detrás de todo, pasa por encima del panel y por debajo del encabezado. Nada se vuelve transparente; el contenido de la página sigue tapado. Sobre una diagonal el panel hereda el plano predominante, que es el mismo criterio con el que la nav elige su tinta.
...
- **El realce es una excepción con nombre, y una sola frase.** `--c-realce`, `oklch(0.36 0.075 87)`: un dorado profundo que vive en "Sitios web pensados desde cero para representar *tu identidad*" y en ningún otro lugar. **La regla que lo vuelve legítimo es que hay una sola línea en todo el sitio que le habla al visitante en segunda persona, y es ésa** — el color marca la frase que te habla a vos. Si aparece en un segundo lugar deja de significar eso y pasa a ser un color de marca. *De dónde sale:* es la piedra concentrada — matiz 87 contra los 91 del plano, así que no intensifica la paleta desde afuera sino desde adentro. *Su croma (0.075) se sale de la disciplina de los planos (0.016–0.025) a propósito*, y queda por debajo del acento que el sistema ya reservaba. *Y su límite, dicho:* 4.07:1 es lo máximo que da un dorado sobre este plano —el único elemento del sitio por debajo de 4.5:1—, y no se arregla oscureciéndolo más: por debajo de L 0.36 con este croma el color se sale de sRGB y deja de ser dorado.
